...
struct EdgeList
{
    ...
    uint16_t getInDegree() const {
        return static_cast<uint16_t>(incoming.size());
    }
    uint16_t getOutDegree() const {
        return static_cast<uint16_t>(outgoing.size());
    }
};
...
struct alignas(64) Node
{
    ...
    Node(Node&& other) noexcept
        : data(std::move(other.data)), generation(other.generation.load()), occupied(other.occupied) {
...
class DirectedAcyclicGraph
{
    ...
    std::queue<uint32_t> _freeList;  ///< A queue of indices for recently freed node slots, enabling reuse and reducing
...
public:
    ...
    bool isHandleValid(const AcyclicNodeHandle<T>& handle) const {
        ...
        return _nodes[index].occupied && _nodes[index].generation.load() == handle.handleGeneration();
    }
    ...
    void removeAllEdges(uint32_t nodeIndex) {
        ...
        for (uint32_t target : outgoing) {
            ...
            targetIncoming.erase(std::remove(targetIncoming.begin(), targetIncoming.end(), nodeIndex),
                                 targetIncoming.end());
        }
        ...
        for (uint32_t source : incoming) {
            ...
            sourceOutgoing.erase(std::remove(sourceOutgoing.begin(), sourceOutgoing.end(), nodeIndex),
                                 sourceOutgoing.end());
        }

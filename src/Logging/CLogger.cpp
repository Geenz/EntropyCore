...
static LogLevel mapLevel(EntropyLogLevelC lvl) noexcept {
    switch (lvl) {
...
static void vwriteInternal(EntropyLogLevelC level, const char* category, const char* fmt, va_list args) {
    if (!fmt) return;
    ...
    if (category && *category) {
        Logger::global().log(mapLevel(level), category, message);
    } else {
        Logger::global().log(mapLevel(level), "C", message);
    }
}

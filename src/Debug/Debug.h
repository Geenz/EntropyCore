...
public:
    ...
    void registerObject(const INamed* object, std::string_view typeName) {
        ...
        _objects[object] = Entry{object, std::string(typeName), std::chrono::system_clock::now()};

        auto msg =
            std::format("Registered {} '{}' at {}", typeName, object->getName(), static_cast<const void*>(object));
    ...
    void unregisterObject(const INamed* object) {
        ...
        if (it != _objects.end()) {
            auto msg = std::format("Unregistered {} '{}' at {}", it->second.typeName, object->getName(),
    ...
    void logAllObjects() const {
        ...
        for (const auto& [typeName, objects] : byType) {
            ...
            for (const auto* obj : objects) {
                auto objMsg = std::format("  - '{}' at {}", obj->getName(), static_cast<const void*>(obj));
...
public:
    ...
    explicit AutoDebugRegistered(std::string_view typeName, Args&&... args) : T(std::forward<Args>(args)...) {

...
#define ENTROPY_DEBUG_ASSERT(condition, message)                                                                    \
    do {                                                                                                            \
        if (!(condition)) {                                                                                         \
            auto msg = ::EntropyEngine::Core::Debug::debugFormat("Assertion failed: {} - {}", #condition, message); \
            ::EntropyEngine::Core::Logging::Logger::global().fatal("Assertion", msg);                               \
 ...
 */
#ifdef EntropyDebug
#define ENTROPY_DEBUG_ONLY(code) \
    do {                         \
        code                     \
    } while (0)
#else
...
public:
    explicit ScopedTimer(std::string_view name, bool logOnDestruct = true)
        : _name(name), _start(std::chrono::high_resolution_clock::now()), _logOnDestruct(logOnDestruct) {}
 ...
 */
#ifdef EntropyDebug
#define ENTROPY_DEBUG_SCOPE(name) ::EntropyEngine::Core::Debug::DebugScope _debugScope##__LINE__(name)
#else
...
[[nodiscard]] bool validatePointer(T* ptr, std::string_view name,
    ...
    if (!ptr) {
        auto msg = debugFormat("Null pointer: {} at {}:{}", name, loc.file_name(), loc.line());

...
uint64_t WorkContractHandle::classHash() const noexcept {
    static const uint64_t hash =
        static_cast<uint64_t>(EntropyEngine::Core::TypeSystem::createTypeId<WorkContractHandle>().id);
    return hash;
}

std::string WorkContractHandle::toString() const {
    if (hasHandle()) {
        return std::format("{}@{}(owner={}, idx={}, gen={})", className(), static_cast<const void*>(this),
                           handleOwner(), handleIndex(), handleGeneration());
    }

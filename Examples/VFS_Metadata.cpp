...
static std::string tp(const std::string& name) {
...
int main() {
    ...
    ListDirectoryOptions single;
    single.globPattern = std::filesystem::path(fh1.metadata().path).filename().string();
    auto singleList = tempDir.list(single);
    singleList.wait();
    if (singleList.status() == FileOpStatus::Complete) {
        ...
        if (!entries.empty()) {
            ...
            ENTROPY_LOG_INFO(entries.front().fullPath + ": size=" + std::to_string(meta.size) +
                             ", exists=" + std::to_string(meta.exists ? 1 : 0));
        }
    }

    // Batch-like metadata via listing of temp directory with glob
    ListDirectoryOptions batch;
    batch.globPattern = std::string("vfs_meta_*.txt");
    batch.sortBy = ListDirectoryOptions::ByName;
    auto batchList = tempDir.list(batch);
    batchList.wait();
    if (batchList.status() == FileOpStatus::Complete) {
        for (const auto& e : batchList.directoryEntries()) {
            ENTROPY_LOG_INFO(e.fullPath + ": exists=" + std::to_string(e.metadata.exists ? 1 : 0) +
                             ", size=" + std::to_string(e.metadata.size));
        }
    }

...
public:
    ...
    std::string toString() const override {
        if (hasHandle()) {
            return std::format("GameObject('{}')#({:p}, idx={}, gen={})", _name, handleOwner(), handleIndex(),
                               handleGeneration());
        } else {
...
int main() {
    ...
    if (auto objHandle = pool.toHandle(player)) {
        ENTROPY_LOG_INFO_CAT("HandleExample", std::format("Object->Handle roundtrip: index={} gen={}",
                                                          objHandle->getIndex(), objHandle->getGeneration()));
    }

    // Erase the enemy; its handle should become invalid after generation bump
    pool.erase(hEnemy);
    auto enemyAfterErase = pool.resolve(hEnemy);
    ENTROPY_LOG_INFO_CAT("HandleExample",
                         std::format("After erase, old enemy handle valid: {}", static_cast<bool>(enemyAfterErase)));
    ...
    ENTROPY_LOG_INFO_CAT(
        "HandleExample",
        std::format("Old enemy handle index/gen = {}/{}; New handle index/gen = {}/{}", hEnemy.getIndex(),
                    hEnemy.getGeneration(), hEnemy2.getIndex(), hEnemy2.getGeneration()));

...
int main() {
    ...
    auto w = srcHandle.writeAll(payload, wo);
    w.wait();
    ...
    auto cw = dstHandle.writeAll(r.contentsBytes(), cwo);
    cw.wait();
    ...
    auto r2 = dstHandle.readAll();
    r2.wait();
    if (r2.status() == FileOpStatus::Complete) {
        WriteOptions mwo;
        mwo.truncate = true;
        mwo.createIfMissing = true;
        auto mw = dst2Handle.writeAll(r2.contentsBytes(), mwo);
        mw.wait();
